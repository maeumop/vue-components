<script setup lang="ts">
  ...
  const parentScrollEvent = (evt: Event): void => {
    isShowOption.value = false;
  };
        ...
        >
          ...
          <ul ref="ul" class="scrollbar">
              ...
              >
                <template v-if="props.multiple">
                  <SvgIcon
                    ...
                    class="checkbox"
                    :path="
                      isOptionSelected(item.value) ? mdiCheckboxMarked : mdiCheckboxBlankOutline
                    "
                  />
                </template>
                {{ item.text }}
            ...
            <li class="items-loading" v-if="props.isLoading">
              <SvgIcon type="mdi" size="24" :path="mdiGoogleCirclesExtended" />
            </li>
          </ul>
