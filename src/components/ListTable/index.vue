...
import type { ListTableEmits, ListTableExpose, ListTableFooter, ListTableHeader, ListTableItem, ListTableProps } from './types';
...
const checkedAllToggle = (bool: boolean = false): void => {
  if (props.checkMode === 'checkbox') {
    ...
    checkedIndexList.value = 
      dataList.value.reduce<number[]>((prev, current, index) => {
...
const checkedItemToggle = (index:number, bool: boolean = false): void => {
  if (index > -1 && index < dataList.value.length) {
    ...
    const checkedArr:T[] = 
        props.items.reduce<T[]>((prev, current, index) => {
    ...
    if (props.items.length && checkedArr.length) {
      ...
      if (props.disableFilter) {
        maxSelectLength = dataList.value
  ...
  if (slots.header && listTable.value) {
    ...
    let result:number = tHead.reduce((prev, current) => {
      ...
      if (current.tagName.toLowerCase() === 'tr') {
        thCount = Array.from(current.children)
          .reduce((pre, el) => {
            let count:number = 0;
